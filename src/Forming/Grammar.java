...
public class Grammar {
	...
	public Grammar(Alfabet _alfabet, String _startSymbol, ProductionRule... _rules) {
		...
		this._nonTerminalSymbols = new Alfabet(
				(String[]) this._rules.stream().map(ProductionRule::getFrom).toArray(String[]::new));
	...
	public boolean isValidString(String str, boolean mustEnd) {
		for (char c : str.toCharArray()) {
			...
			if (!_alfabet.hasSign(Character.toString(c)))
				return false;
			...
			// Get Start Rule
			Optional<ProductionRule> start = _rules.stream()
					.filter((ProductionRule pr) -> pr.getFrom().equals(this._startSymbol)).findAny();
			...
			while (!start.isEmpty()) {
				...
				Optional<TransitionRule<String>> rule = start.get().getTransitions().stream().filter(
						(TransitionRule<String> pr) -> pr.getSign().equals(Character.toString(str.charAt(counter[0]))))
						.findAny();
				...
				if (!rule.isEmpty()) {
					...
					start = _rules.stream()
							.filter((ProductionRule pr) -> pr.getFrom().equals(rule.get().getGoTo())).findAny();
				} else {
					int highestMatch = 0;
					String[] newString = { "" };
					...
					if (highestMatch < 1) {
						start = _rules.stream()
								.filter((ProductionRule pr) -> pr.getFrom().equals(this._startSymbol))
								.findAny();
					} else {
						start = _rules.stream()
								.filter((ProductionRule pr) -> pr.getFrom().equals(this._startSymbol))
						...
						for (int[] count = { 0 }; count[0] < highestMatch; count[0]++) {
							...
							rule2.set(0,
									start.get().getTransitions().stream()
											.filter((TransitionRule<String> pr) -> pr.getSign()
													.equals(Character.toString(newString[0].charAt(count[0]))))
											.findAny());

							if (!(rule2.get(0) == null) && !rule2.get(0).isEmpty()) {
								start = _rules.stream().filter((ProductionRule pr) -> pr.getFrom()
										.equals(rule2.get(0).get().getGoTo())).findAny();
							}
						}
					}
				}
...
//		}
	...
	private String[] get_end_states() {
		return this._rules.stream().filter(pr -> pr.isEndState()).map(ProductionRule::getFrom).toArray(String[]::new);
	}

	public String toGrammarString() {
		StringBuilder str = new StringBuilder();
		for (ProductionRule rule: _rules) {
			str.append(rule.toProductionRuleString(get_end_states())+"\n");
		}
